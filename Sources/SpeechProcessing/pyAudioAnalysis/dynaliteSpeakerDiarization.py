...
def speakerDiarization(fileName, numOfSpeakers):
	'''
	ARGUMENTS:
	...
	# In the original program, the following were paremeters
	# mtSize=2.0, mtStep=0.2, stWin=0.05, LDAdim=35, PLOT=False
	# I decided to hardcode them here
	mtSize=2.0
	mtStep=0.2
	stWin=0.05
	LDAdim=0
	...
	if PLOT:
		...
		# In the original program, the plot would be shown
		# plt.show()

		# We just want the (x, y) values
		ax = plt.gca()
		xvals = ax.lines[0].get_xdata()
		yvals = ax.lines[0].get_ydata()

		# Make a list of y-x segments in the form (speakerID, timestamp)
		segments = zip(yvals, xvals)

		speaker_segments = []
		if len(segments) > 1:
			start_seg = segments[0]
			for seg_index in range(1, len(segments)):
				if segments[seg_index][0] != start_seg[0]:
					pass
